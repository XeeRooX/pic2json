...
repo_root = os.path.dirname(os.path.abspath(__file__))[:os.path.dirname(os.path.abspath(__file__)).find("pic2json")+8]
...
def display_result(json_data, html_file, show_json, show_html):
	...
	buttons_str = f"{download_json_btn_str}"
...
def submit_clicked(value, uploaded_img_file, options, show_json, show_html):
	if(value):
		with st.spinner(text='Submitted successfully. Processing image...'):
			...
			display_result(json_data, None, show_json, show_html)
...
def main():
	...
	uploaded_file = st.file_uploader("Выберите изображение", type=["png", "jpg", "JPG", "jpeg"])
	...
	options["cnn_model"] = "cnn-rico"
	
	options["min_grad"] = 3 # st.sidebar.slider("Gradient Threshold to produce Binary Map", min_value=1, max_value=10, value=3, step=1)
	options["ffl_block"] = 5 #st.sidebar.slider("Fill-flood threshold", min_value=1, max_value=10, value=5, step=1)
	options["min_ele_area"] = 35 #st.sidebar.slider("Minimum Area for Components", min_value=10, max_value=200, value=25, step=1)
	options["merge_contained_ele"] = False # st.sidebar.checkbox("Merge Elements Contained in Others")

	#st.sidebar.subheader("For Text:")
	options["max_word_inline_gap"] = 4 #st.sidebar.slider("Max Text Word Gap to be Same Line", min_value=1, max_value=15, value=4, step=1)
	options["max_line_gap"] = 4 #st.sidebar.slider("Max Text Line Gap to be Same Paragraph", min_value=1, max_value=20, value=4, step=1)

	submitted = st.button("Преобразовать")
